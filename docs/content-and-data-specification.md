...
MVPカテゴリは空港、ワルン、コンビニ、Grabとする。Phase 4で病院、薬局、銀行、SIM・通信、
配送・郵便、家、警察、VISA・行政、職場を追加した。将来カテゴリはホテル、レストラン、病院、薬局、銀行、SIM・通信、e-money、家・不動産、配送・郵便、警察、VISA・行政、税務署、学校、会社、工場、市場、美容院、修理、災害、ニュース・時事である。
...
アプリ側は次を実装済みである。教材管理システム本体（レビュー画面・承認操作・配信）はアプリ外。

- 問題が級・形式・公開状態・誤答理由を持つ
- `published` のみを新規セッションへ出題し、`deprecated` は過去参照のみ許す
- 自動検査（正解の一意性、選択肢の重複、誤答理由の有無、未提供の級・形式、チェックサム）
- 教材パックのチェックサム算出と検証
- 検査を通った版だけを有効化し、落ちた版では現在の有効版を保持する
- 教材の版・チェックサム・出題可能数・検査結果を設定画面に表示する
...
iOSクライアントは管理APIへアクセスしない。B〜A級のテキストAI評価は採点中継APIを利用する。スピーチと面接の音声回答は `recording-policy.md` に従って端末内で文字起こしし、確認済みテキストだけを提出する。面接の筆記回答は文字起こしせず入力テキストを確認する。両モードともインドネシア語の質問音声が起点で、日本語訳は操作時のみのヒントとする。面接の提出設問は実際に読み上げた質問本文と一致させる。音声アップロードはこの方針に含めない。現在の提供状況は `delivery-plan.md` のPhase 5を参照する。
...
現在の提供状況・未実装項目・実測結果への参照は `delivery-plan.md` のPhase 5に集約する。評価契約は次を含む。

- ルーブリック（版付き）と検証、正規化スコアの算出
- 提出の受け付け判定（録音・空文・基準不一致を拒否）
- 再評価キュー（一時失敗は保持、恒久的な不備は理由付きで除外）
- 人間レビューの状態遷移（AI採点だけを最終結果にしない）
- 基準回答の回帰試験（期待範囲を外れたモデルへ切り替えない）
