...
- 問題画面: **配置しない**。音声操作と選択肢を同時に見せることを優先し、
  キャラクターの表示面積で選択肢を押し下げない。
...
- ライト／ダーク背景での縁取り規則
- 権利、生成元、利用条件の記録
- **口の開閉差分**（通常ポーズの口を閉じた状態・開いた状態）

現在の3素材は口の開閉差分を持たないため、音声再生中の口の動きは
`LearningCompanionView` が通常ポーズの口の位置へ描画するオーバーレイで代替している。
口の開閉素材が用意でき次第、オーバーレイを素材の差し替えへ置き換える。

### 素材の投入方法

表示側は6表情ぶんの素材名を先に決めてある。Asset Catalog に下表の名前で追加すると、
コードを変更せずにそちらが使われる。素材が無い間は「代替」の素材で表示する。

| 表情 | 正式素材名 | 代替 | 状態 |
| --- | --- | --- | --- |
| neutral | `CompanionNormal` | `CompanionListening` | 未投入 |
| happy | `CompanionHappy` | — | 投入済み |
| surprised | `CompanionSurprised` | `CompanionEncourage` | 未投入 |
| thinking | `CompanionThinking` | `CompanionListening` | 未投入 |
| celebrate | `CompanionCelebrate` | `CompanionHappy` | 未投入 |
| encourage | `CompanionEncourage` | — | 投入済み |

未投入の素材は `assets/generated-character-assets.md` の共通生成指示に、
次のポーズ指示を足して生成する。

| 素材名 | 追加するポーズ指示 |
| --- | --- |
| `CompanionNormal` | 正面、口を閉じた穏やかな微笑み、両手を下ろす |
| `CompanionThinking` | 正面、片方の前足を口元へ、視線をわずかに上へ |
| `CompanionSurprised` | 正面、目を丸く見開き、口を小さく開ける |
| `CompanionCelebrate` | 正面、片目を閉じて片方の前足を上げる |

口パクを素材で行う場合は、`CompanionNormal` の口を開いた差分も同時に用意する。
...
キャラクターは静止画として置かない。伴走者に見えることが要件である。

| 状態 | 動き |
| --- | --- |
| 待機 | 2.4秒周期で息づく（拡大率 0.975〜1.03、下端固定） |
| 音声再生中 | 0.34秒周期の上下の揺れに加え、発話の拍ごとに弾む |
| 音声再生中 | 発話の拍に合わせて口を開閉し、口元の両側に音波を出す |

キャラクターを出すのは、初回導入・ホーム・結果など、表示面積に余裕がある画面に限る。
問題画面では再生ボタン自体を波形で animate させ、キャラクターは置かない。
| Reduce Motion | 息づき・揺れ・傾きを止め、口は開いた状態を保持しない |

発話の拍は音声側から届く発話区切りイベント（`QuestionAudioPlaying.onSpeechMark`）で、
合成音声では語の開始ごとに届く。実音声アセットへ差し替えるときは、
同じイベントを時間軸マーカーから発火させる。

動きは学習の妨げにならない範囲に収め、問題文・選択肢・音声操作を隠さない。
