...
- 同期の状態（接続、未送信の回答数）と、オフラインでも学習できる旨を示す
- 保存している範囲（端末内のみ・外部送信なし・録音なし）を明示する
- 学習データの削除導線を置く。削除は確認してから実行する
...
![C級カテゴリと実戦チェック](screenshots/scenario-detail-practical.png)
...
- C級カテゴリには実戦チェック（5問・8割で合格）を置く。直近の結果と再挑戦の導線を示す
- 実戦チェックが必要な行動は、問題を習得していてもその旨を明示する
...
| 教材 | E級40問 + D級20問 + C級45問。公開済みのみ出題し、自動検査とチェックサムで破損を拒否 |
| 実戦チェック | C級9カテゴリで実施。合格すると実戦が必要な行動が解放される |
| オフライン | 教材同梱のため通信なしで20問を完了できる |
| 同期 | 回答は冪等キー付きでキューに積み、受理されたものだけ外す。再接続で自動送信 |
| 性能 | 問題表示1秒・音声開始500msを目標として実測を記録する |
